import Page from 'src/components/page'
import EventCard from 'src/components/event-card'
import Loader from 'src/components/loader'
import { compareEvents } from 'src/utils/compare-events'
import { useGeoLocation } from 'src/utils/use-geo-location'
import { useEvents } from 'src/cache/events/use'
import { useState } from 'preact/hooks'
import TextInput from 'src/components/text-input'
import { css } from '@linaria/core'
import { Dropdown } from 'src/components/dropdown'
import { useQueryState } from 'src/utils/use-query-state'
import { UnstyledList } from 'src/components/unstyled-list'
import { useYears } from 'src/utils/use-years'
import IconButton from 'src/components/icon-button'
import { mdiCrosshairsGps } from '@mdi/js'

const homeStyle = css`
  display: grid;
  justify-content: center;
  margin: 1rem;
  grid-gap: 1rem;
  grid-template-columns: minmax(auto, 23rem);
`

const matchListStyle = css`
  display: grid;
  grid-gap: 1rem;
`

const filterStyle = css`
  display: flex;
  align-items: center;
  margin: -0.5rem;

  & > * {
    margin: 0.5rem;
  }

  & > :nth-child(2) {
    flex-grow: 1;
  }
`

const now = new Date()
const Home = () => {
  const [location, prompt] = useGeoLocation()
  const [query, setQuery] = useState('')
  const lowerCaseQuery = query.toLowerCase()
  const years = useYears().sort().reverse()
  const [yearVal, setYear] = useQueryState('year', years[0])
  const year = Number(yearVal)
  const events = useEvents(year)

  return (
    <Page name="Home" back={false} class={homeStyle}>
      <div class={filterStyle}>
        <Dropdown options={years} onChange={setYear} value={year} />
        <TextInput onInput={setQuery} label="Search for Events" />
        {prompt && (
          <IconButton
            icon={mdiCrosshairsGps}
            onClick={prompt}
            title="Use geolocation for sorting"
          />
        )}
      </div>

      {events ? (
        <>
          <UnstyledList class={matchListStyle}>
            {events
              .filter((event) => {
                if (!query) return true
                return (
                  event.name.toLowerCase().includes(lowerCaseQuery) ||
                  event.key.toLowerCase().includes(lowerCaseQuery) ||
                  event.locationName.toLowerCase().includes(lowerCaseQuery) ||
                  event.district?.toLowerCase().includes(lowerCaseQuery) ||
                  event.fullDistrict?.toLowerCase().includes(lowerCaseQuery)
                )
              })
              .sort(compareEvents(now, location))
              .slice(0, 20) // Displaying just the first 20 to improve rendering/re-rendering performance (esp. while searching)
              .map((e) => (
                <li key={e.key}>
                  <EventCard event={e} />
                </li>
              ))}
          </UnstyledList>
          <a
            href="https://www.netlify.com"
            class={css`
              margin: 1rem auto;
            `}
          >
            <img
              alt="Deploys by Netlify"
              src="https://www.netlify.com/img/global/badges/netlify-color-accent.svg"
            />
          </a>
        </>
      ) : (
        <Loader />
      )}
    </Page>
  )
}

export default Home
